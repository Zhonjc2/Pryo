...
public class Response implements Serializable {
    ...
    //0为登录成功并返回笔记列表notesVector，1为注册成功，2为返回笔记，3为保存成功，4为添加成功，6为笔记列表获取成功返回笔记列表，7为断开成功，8为删除笔记成功
    //-1为登录失败密码错误，-2为注册失败SQL错误，-3为注册失败用户已存在，-4为笔记返回异常，-5为保存异常，-6为添加异常，-7为删除笔记失败，-8为笔记列表获取失败，-9为断开失败，-10参数错误
